...
%     invJ = subsVar_3DOF(invJv,feedback(end,:));
    ...
    Q_dot = inverseJ(feedback(end,:),Xc_dot,movementType);
    sendJointVel(s, [Q_dot', 0], numID);
    dt = toc(time)
    time = tic;
...
newAngle(1) = -angle(1);
newAngle(2:(length(angle))) = angle(2:(length(angle))) - offset(1:(length(angle)-1));
end
...
function Q_dot = inverseJ(Q,xc,movement_type)
[d1,d2,d3,d4] =  RobotSpec_Assembly();
...
if (movement_type == 'x')
    Q_dot = [-1*( cos(Q1)/(d3*cos(Q2 + Q3) + d2*cos(Q2))*xc(2) + xc(3)*0);...
              xc(2) * (cos(Q2 + Q3)*sin(Q1))/(d2*sin(Q3)) + xc(3) *  sin(Q2 + Q3)/(d2*sin(Q3));...
             xc(2) * -(sin(Q1)*(d3*cos(Q2 + Q3) + d2*cos(Q2)))/(d2*d3*sin(Q3)) + xc(3) * -(d3*sin(Q2 + Q3) + d2*sin(Q2))/(d2*d3*sin(Q3))];
else
    Q_dot = [-1*(-sin(Q1)/(d3*cos(Q2 + Q3) + d2*cos(Q2))*xc(1));...
             (cos(Q2 + Q3)*cos(Q1))/(d2*sin(Q3))*xc(1) +  xc(3) *  sin(Q2 + Q3)/(d2*sin(Q3));...
             xc(1)*-(cos(Q1)*(d3*cos(Q2 + Q3) + d2*cos(Q2)))/(d2*d3*sin(Q3)) + xc(3) * -(d3*sin(Q2 + Q3) + d2*sin(Q2))/(d2*d3*sin(Q3))];
end
end
